...
{
        if(servo_left_angle < 43) servo_left_angle = 43;    // previously 10
        if(servo_left_angle > 180) servo_left_angle = 180;  // previously 120
        if(servo_right_angle < 10) servo_right_angle = 10;    // previously 10
        if(servo_right_angle > 120) servo_right_angle = 120;  // previously 110

        if(servo_left_angle + servo_right_angle > 209)    // previously 160
        {
                servo_right_angle = 200 - servo_left_angle;
                return FAILED;
        }
...
int uArmClass::move_to(double x, double y, double z, double hand_angle, byte relative_flags, double time, byte path_type, byte ease_type, boolean enable_hand) {
        ...
        {
            //alert(50, 10, 10);
            Serial.write("Failed: servo not liearly calibrated");
            return FAILED;
        }
